...
__Complying with the SensirionSCD30 sensors transmission speed limitation__  
...
_Reducing the I2C bus clock to 32 kHz and included CRC check on the received data in order to only do floating point handling of "good" data._
...
This seems to be some what unwise, since the attempt to upload the WEMOS D1 mini from Lolin, with the ESP8266 NONOS firmware for AT operation failed.  
Will stick to the Arduino platform (NodeMCU) for making the D1 mini a IoT device.  
Focus will be on Preparing for toy shooting target hit indication, which is a different project.  


## Flashing STM32 MCUs over USB (PDU device)
This is a interesting feature that should be investigated.  
This is an option that can be utilized if no USB-to-serial hardware is available.  
_Requirements:_  
HSE (high Speed External clock) must be enabled, meaning that a external oscillator must be connected to the mcu's OSC_IN and OSC_OUT pins. see AN2867
Application note file:///C:/Users/dagak/Downloads/cd00221665-oscillator-design-guide-for-stm8af-al-s-stm32-mcus-and-mpus-stmicroelectronics.pdf  
__Note!__ There is a spot on the NUCLEOF302R8 board for this crystal oscillator marked as X3 on the beneath the mcu if the st-link part of the board is viewed on top.  
In the schematics this spot is prepared for a 8 MHz crystal oscillator, which is to provide the HSE for USB boot and programming the on-chip flash.  

However, this is for now of little interest, as the same option is available for one of the serial ports.  


__Study__  
AN2606 Application note  
STM32 microcontroller system memory boot mode  
Will try to upload binary to flash memory through USART1 from STM32CubeProgrammer. The USART1 is (has to be) enabled through st-link.
Question is, what happens if the binary does not include the USART enable software? Will it disable the option of uploading new binaries onto the stm32 flash?  
